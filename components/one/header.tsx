...
// Removed unused AnimatedLogo import
...
const FAQ_ITEMS = [
  {
    value: "item-0",
    question: "Is NULL.PROTOCOL a mixer?",
    answer: (
      <>
        <p>No.</p>
        <p className="mt-3">
          NULL.PROTOCOL is not a mixer and does not rely on pooling or
        ...
        <p className="mt-3">
          ...
          our underlying TEE (MagicBlock's ephemeral rollup). This design is intended to obscure
          the direct on-chain link between sender and recipient while supporting
          compliance through permissioned access and risk screening.
        </p>
      </>
    ),
  },
  {
    value: "item-1",
    question: "Are private resource payments truly private?",
    answer: (
      <>
        <p>
          Our platform is designed to provide strong privacy for AI agent resource allocation,
          but not absolute anonymity.
        </p>
        <p className="mt-3">
          The system obscures the direct on-chain link between your wallet and the
          compute resource node by separating deposit and payout flows and executing
  ...
  },
  {
    ...
    answer: (
      <>
        <p>No.</p>
        <p className="mt-3">
          User funds remain controlled by Solana smart contracts powered by MagicBlock. The vault
          operates according to predefined program logic. NULL.PROTOCOL does not take
          discretionary control over your digital assets.
        </p>
      </>
    ),
  },
  {
    value: "item-3",
    question: "How does the system support compliance?",
    answer: (
      <>
        <p>We are designed to support compliance through the underlying framework:</p>
        <ul className="mt-3 list-disc space-y-1 pl-5">
          <li>Geo-fencing & Policy-based Access Controls</li>
          <li>AML risk screening of wallets and transactions</li>
          <li>Hardware-level attestation (Intel TDX)</li>
        </ul>
      </>
    ),
  },
  {
    value: "item-4",
    question: "What trust assumptions does NULL.PROTOCOL rely on?",
    answer: (
      <>
        <p>
          We rely on Secure Private Ephemeral Rollups combining execution within the latest Intel Trusted Domain Extension (TDX) with onchain permission access to process private intents securely.
        </p>
        <p className="mt-3">Users should consider:</p>
        <ul className="mt-3 list-disc space-y-1 pl-5">
          ...
          <li>The security guarantees of the hardware manufacturer (Intel TDX via MagicBlock)</li>
        </ul>
      </>
    ),
  },
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between bg-background/80 px-4 py-4 backdrop-blur-md sm:px-6">
      {/* Logo */}
      <Link href="/" aria-label="NULL PROTOCOL" className="flex items-center gap-2 group">
        {/* Custom SVG Logo derived from the centerpiece */}
        <svg width="28" height="28" viewBox="0 0 100 100" className="text-[#d1fb00] group-hover:drop-shadow-[0_0_10px_rgba(209,251,0,0.8)] transition-all">
          <path 
        ...
        <div className="flex items-center gap-1.5 ml-1">
          <span className="font-mono text-foreground font-bold tracking-[0.2em] text-sm sm:text-base">
            NULL<span className="text-[#d1fb00]">.PROTOCOL</span>
          </span>
        </div>
      </Link>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        */}
        ...
        >
          ...
          <span className="hidden sm:inline">MagicBlock Framework</span>
        </a>
        ...
        >
          ...
          <span className="hidden sm:inline">Underlying TEE Specs</span>
        </a>
        ...
        <Dialog open={faqModalOpen} onOpenChange={setFaqModalOpen}>
          <DialogContent className="max-h-[80vh] max-w-2xl overflow-hidden border border-[#d1fb00]/20 bg-[#050505] shadow-[0_0_40px_rgba(209,251,0,0.05)] p-0 gap-0 font-mono">
            <DialogHeader className="border-b border-[#d1fb00]/20 bg-[#0a0a0a] px-6 py-5">
              <DialogTitle className="text-[#d1fb00] font-bold tracking-[0.2em] uppercase text-lg">SYSTEM.FAQ</DialogTitle>
              <DialogDescription className="text-white/40 text-xs uppercase tracking-widest mt-1">
              ...
              >
                {FAQ_ITEMS.map((item) => (
                  <AccordionItem key={item.value} value={item.value}>
                    <AccordionTrigger className="text-sm tracking-wide text-white hover:text-[#d1fb00] hover:no-underline text-left">
                      {item.question}
                    </AccordionTrigger>
                    <AccordionContent className="text-xs leading-relaxed text-white/50 tracking-wide font-sans">
                      {item.answer}
                    </AccordionContent>
                  </AccordionItem>
                ))}
